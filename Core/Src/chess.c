...
#define HASHSIZE 0x1008

#include <stdio.h>

// Define a struct to hold square and input
struct SquareInput
{
    char input;
    char square[3]; // For example, "a1", "b2", etc.
};

// Initialize an array of structs directly
struct SquareInput nodes[] = {
    {112, "a1"},
    {96, "a2"},
    {80, "a3"},
    {64, "a4"},
...
// Function to return square from input
static char *squareFromInput(char input)
{
    for (int i = 0; i < 64; i++)
    {
        if (nodes[i].input == input)
        {
            return nodes[i].square;
        }
    }
    return NULL; // Return "Not found" if input is not found
}
...
static int V = 112, M = 136, S = 128, INF = 8e3, C = 799, /* V=0x70=rank mask, M=0x88 */
    Rootep, Nodes, i;

static char RootEval, InputFrom, InputTo,
...
static int Search(int Side, int Alpha, int Beta, int Eval, int HashKeyLo, int HashKeyHi,
                  int epSqr, int LastTo, int Depth)
{
    ...
    if (Hash->Key)     /* hit: pos. is in hash tab */
    {
        ...
        IterDepth = BestFrom = BestTo = 0; /* start iter., no best yet */
    ...
    while (IterDepth++ < Depth |                       /* iterative deepening loop */
           LastTo == 8 & Nodes < 1e4 & IterDepth < 98) /* at root until node limit */
...
void make_move(int Side, char *output_move)
{
    ...
    char *from = squareFromInput(InputFrom);
    char *to = squareFromInput(InputTo);
    output_move[0] = from[0];
    output_move[1] = from[1];
    output_move[2] = to[0];
    output_move[3] = to[1];
    return;
}
...
void start_game(int color)
{
    ...
    for (i = 0; i < 8; i++) /* initial board setup*/
    {
        ...
        Board[i + 0x10] = 18;
        Board[i + 0x60] = 9; /* black, white Pawns */
        for (j = 0; j < 8; j++)
            Board[16 * j + i + 8] = (i - 4) * (i - 4) + (j - 3.5) * (j - 3.5);
